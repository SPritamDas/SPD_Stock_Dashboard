...
(function () {
  try {
    ...
    if (S.closed === undefined) S.closed = true;           // start CLOSED on first load (open via the 🧮 bubble)
...
(function () {
  try {
    ...
    if (UI.closed === undefined) UI.closed = true;      // start CLOSED on first load (open via the 📒 bubble)
